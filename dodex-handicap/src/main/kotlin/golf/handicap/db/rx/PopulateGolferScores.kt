...
class PopulateGolferScores : SqlConstants(), IPopulateGolferScores {
        ...
        pool!!
            ...
            .doOnSuccess { conn ->
                conn.rxBegin().doOnSuccess { tx ->
                    ...
                    parameters.addDouble(golfer.handicap)
                        ...
                        .doFinally {
                            ...
                            if (score == null) {
                                ...
                                conn.preparedQuery(sql)
                                    .rxExecute(parameters)
                                    .doOnSuccess {
                                        rows -> rowsUpdated += rows.rowCount()
                                        tx.commit()
                                    }
                                    .doOnError { err ->
                                        LOGGER.severe(
    ...
    override fun getGolferScores(golfer: Golfer, rows: Int): Uni<Map<String, Any?>>? {
        val oldRows = this.maxRows
        val golferScores: io.vertx.mutiny.core.Promise<Map<String, Any?>>? = io.vertx.mutiny.core.Promise.promise()
        this.maxRows = rows
        gettingData = true
    ...
    @Throws(Exception::class)
    override fun removeLastScore(golferPIN: String?): Uni<String> {
        val usedPromise: io.vertx.mutiny.core.Promise<String> = io.vertx.mutiny.core.Promise.promise()
        var count = 0
