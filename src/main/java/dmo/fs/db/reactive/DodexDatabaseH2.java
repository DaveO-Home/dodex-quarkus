  ...
  public Promise<JDBCPool> databaseSetup() {
    ...
    Completable completable = pool.rxGetConnection().flatMapCompletable(conn -> conn.rxBegin()
        .flatMapCompletable(tx -> conn.query(CHECKUSERSQL).rxExecute().doOnSuccess(row -> {
            ...
            .flatMap(result -> conn.query(CHECKGROUPSSQL).rxExecute()
                .doOnSuccess(row -> {
                  RowIterator<Row> ri = row.iterator();
                  String val = null;
                  while (ri.hasNext()) {
                  ...
                  if (val == null) {
                    ...
                    Single<RowSet<Row>> crow = conn.query(sql).rxExecute()
                        .doOnError(err ->
                            logger.info(String.format("Groups Create Table Error: %s", err.getMessage())))
                        .doOnSuccess(row2 -> logger.info("Groups Table Added."));
                    crow.subscribe();
                  }
                }).doOnError(err -> logger.info(String.format("Groups Check Table Error: %s", err.getMessage()))))
                ...
                .doOnSuccess(row -> {
                  ...
                  if (val == null) {
                    ...
                    Single<RowSet<Row>> crow = conn.query(sql).rxExecute()
                        ...
                        .doOnSuccess(row2 -> logger.info("Member Table Added."));

                    crow.subscribe(result2 -> {
                      conn.rxClose().doOnSubscribe(r -> tx.rxCommit().subscribe()).subscribe();
                    }, err -> logger.info(String.format("Member Table Error: %s", err.getMessage())));
                  } else {
                    conn.rxClose().doOnSubscribe(r -> tx.rxCommit().subscribe()).subscribe();
                  }
                }).doOnError(err -> logger.info(String.format("Member Check Table Error: %s", err.getMessage()))))
