...
public abstract class DbFirebaseBase {
    ...
    public Promise<SpaLogin> removeLogin(SpaLogin spaLogin) throws InterruptedException, ExecutionException {
        ...
        try {
            ApiFuture<WriteResult> writeResult = dbf.collection("login")
                    .document(String.format("%s%s", spaLogin.getName(), spaLogin.getPassword())).delete();
            writeResult.get();
